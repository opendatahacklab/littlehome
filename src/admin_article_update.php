...
$a=new Article();
if (isset($_GET['updateURI'])){
	$u=new LDUtils();
	if ($u->isAbsoluteUrl($_GET['updateURI']))
		$oldURI=$_GET['updateURI'];
	else
		$oldURI='../'.$_GET['updateURI'];
	$a->readFromFile($oldURI);
	$_SESSION['oldURI']=$_GET['updateURI'];
} else 
	$a->readFromSession();
...
<html lang="it">
<head>
	<title>LittleHome - Amministrazione Sito - Modifica Articolo</title>
	<meta charset="UTF-8" />
